...
{
	CreateBasicGeom(vertices, vertexCount, indices, indexCount, device);
}
Mesh::Mesh(const char* file, ID3D11Device* device)
{
	...
	obj.close();
	CreateBasicGeom(&verts[0], (UINT)verts.size(), &indices[0], vertCounter, device);
...
void Mesh::CreateBasicGeom(Vertex *vertices, UINT vertexCount, UINT *indices, UINT indexCount, ID3D11Device *device)
{
